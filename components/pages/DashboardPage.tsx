...
export default function DashboardPage() {
  ...
  const handleScan = async (decodedText: string) => {
    ...
    if (dbError) {
      console.error("Supabase check-in error:", dbError);
    }

    // 2. Update Google Sheets via API route
    try {
      const sheetsRes = await fetch("/api/sheets/checkin", {
        method: "POST",
      ...
      if (!sheetsRes.ok) {
        console.error("Google Sheets check-in failed:", await sheetsRes.text());
      }
    } catch (err) {
      console.error("Google Sheets check-in error:", err);
    }

    // 3. Update local state
    setUser({ ...user, last_checkin: new Date().toISOString() });
      ...
      <div className="w-full max-w-md mx-auto py-10 animate-fade-in">
        ...
        <div className="flex justify-between items-center mb-10 px-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold font-montserrat tracking-tight text-gray-800">
                JumpIn
              </h1>
              <p className="text-[10px] text-orange-400 font-bold tracking-widest uppercase -mt-1">
                Dashboard
              </p>
            </div>
          </div>
            ...
            <div className="w-full space-y-3.5 text-left">
              <div className="flex items-center gap-5 p-4 rounded-3xl bg-white/40 border border-white/60 group transition-all hover:bg-white/60">
                ...
                <div className="flex-1">
                  <p className="text-[10px] uppercase tracking-[0.15em] text-gray-400 font-bold mb-0.5">
                    Studente
                  </p>
        ...
        <div className="flex flex-col items-center justify-center space-y-6">
          {success ? (
            ...
            >
              <div
                className={`w-28 h-28 rounded-[3rem] flex items-center justify-center text-white mb-6 transition-all duration-500 ${
                  isProcessing
